...
```{r trim}
dtm_reduced <- removeSparseTerms(dtm, sparse = 0.999)  # Retain terms present in at least 0.1% of documents, so at least 50 reviews
...
Now, let's look at the term distribution. Note that all of these terms (words) appear in at least 50 documents (reviews), but how many times in each document does a term appear?
...
myCorpus <- tm_map(myCorpus, removeWords,c("one", "also", "may","something","around","got","movies","films","made","make","thats","ive"))
