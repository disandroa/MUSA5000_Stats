...
nrc_sentiment <- get_nrc_sentiment(review_long1$Term)
```

```{r warning=FALSE, message=FALSE, cache=FALSE}
review_long1Sentiment <- cbind(review_long1, nrc_sentiment)
```
...
```{r warning=FALSE, message=FALSE, cache=FALSE}
# Select the columns to be multiplied (last ten columns)
cols_to_multiply <- names(review_long1Sentiment)[3:12]

# Multiply the last ten columns (sentiments) by the first column (Term_Frequency)
review_long1Sentiment[, cols_to_multiply] <- review_long1Sentiment[, cols_to_multiply] * review_long1Sentiment$Term_Frequency
```

Now, let's see the total prevalence of each sentiment in the text by summing each column and creating a bar plot

```{r warning=FALSE, message=FALSE, cache=FALSE}
review_long1Sentiment_Total <- t(as.matrix(colSums(review_long1Sentiment[,-1:-2])))
barplot(review_long1Sentiment_Total, las=2, ylab='Count', main='Sentiment Scores')
```
...
review_long1$bing <- as.matrix(get_sentiment(review_long1$Term, method="bing"))
