...
In multiple regression, $\sigma^2$ is calculated using the following formula where $n$ is the number of observations and $k$ is the number of predictors.
...
We use this RMSE to compare it to RMSE of different models to choose the best model (which we define as the model with the smallest RMSE).
...
We used R for all analyses [@rsoftware].
...
After log-transformation, the dependent variable looks more normally distributed. The same is true for the transformed number of households below the poverty level. The other variables have a large spike at zero after being log-transformed. Our regression will use the log-transformed dependent variable `LNMEDHVAL`, and the log-transformed number of households below the poverty level LNNBELPOV100. The remaining independent variables (`PCTBACHMOR`, `MEDHHINC`, `PCTVACANT`, `PCTSINGLES`) will remain untransformed in our regression. Other regression assumptions will be examined in a separate section below [Regression Assumption Checks].
...
The choropleth map of the dependent variable `LNMEDHVAL` is similar to that of `PCTBACHMOR.` `LNNBELPOV` and `PCTVACANT` look like the opposite of `LNMEDHVAL`, suggesting a possible negative relationship between those predictors and the dependent variable. Based on the visualizations, none of the predictors appear strongly inter-correlated with one other. The relationships observed from these chloropleth maps were between predictors and the dependent variable, not within the predictors themselves. Thus, we do not anticipate severe multicollinearity to be an issue in our analysis. 
...
The correlation matrix above supports the earlier conclusion that there is not severe multicollinearity among our predictors. Correlation values between variables are low, all falling within the absolute value range of 0.15 and 0.32. 
...
The quality of the model is quite satisfactory, featuring an R-squared value of approximately 0.66. This indicates that roughly two-thirds of the variation in median house values can be accounted for by our model. Additionally, the F-test yielded a very low p-value, suggesting that it is highly improbable for a model without any predictor variables to achieve such accurate predictions of median house values.
...
We regressed log-transformed median household value (`LNMEDHVAL`) on the % of vacant housing units, % of single family houses (`PCTSINGLES`), % of residents with at least a bachelor’s degree (`PCTBACHMOR`), % of vacant housing units (`PCTVACANT`), and the log-transformed number of households living in poverty (`LNNBELPOV100`). The regression output tells us that each variable is highly significant (p < 0.001 for each variable). `LNNBELPOV100` and `PCTVACANT` are both negatively associated with `LNMEDHVAL.` As median house value changes by 1%, the expected value of `LNNBELOV` changes by approximately -0.77%, holding all else constant (since the $\beta$ coefficient for `LNNBELPOV100` is small (0.789), we can approximate (1.01 $\beta$ − 1) ∙ 100% ≈ $\beta$%. Our coefficient for `PCTVACANT` is -0.019, also a small value (β≤ 0.3 in absolute value), tells us that as median house value goes up by one unit, the expected value of `PCTVACANT` changes by approximately -1.9% (100β%) holding all other variables constant. `PCTSINGLES` and `PCTBACHMOR` are both positively associated with `LNMEDHVAL` with small coefficients. As median house value goes up by one unit, the expected value of `PCTSINGLES` changes by 0.2%, all other variables constant. `PCTBACHMOR` changes by approximately 2% when median house value goes up by one unit.

The model’s $R^2$ and Adjusted $R^2$ are 0.662 and 0.661, respectively. This tells us that approximately 66%, or two thirds of the variance in the dependent variable is explained by the model.
...
The above scatter plots visualize the relationship between the dependent variable, the log-transformed median house value, on the y-axis, and each of our predictors on the x-axis. The relationship between house values and the percentage of block residents with at least a bachelor’s degree is positive and somewhat linear, but the relationship is not strong. The plots for vacant units and the log-transformed number of households in poverty show a negative association with median house values, but not a strong linear relationship. The plot of percentage of single family homes is not linear, and does not show a clear relationship to the dependent variable.

### Standardized Residuals

We standardize the regression residuals by dividing them by their standard error, making it easier to compare residuals to each other and identify potential outliers. The histogram below shows that the standardized residuals follow a normal distribution. 
...
[add outlier chloropleth map here]

To identify which data points are outliers, we used the absolute value of the standardized residuals. Observations where the absolute value of the standardized residual is 4 or greater are considered extreme outliers in our data set. A map of these observations reveals that these outliers are located near a large park, City Hall, a gas plant, and other non-residential uses. Unique neighborhood characteristics for blocks located near one of these sites could explain the high standardized residuals. This context is useful for understanding our model, but does not prompt any additional action.

The maps of the dependent variables and predictors did show spatial autocorrelation. The map of the dependent variable showed blocks with high home values next to other blocks with high home values, and vice versa. Maps of independent variables indicated that blocks with a high percentage of people with bachelor’s degrees were close to each other, and that blocks with high poverty are not usually near blocks with little poverty. Overall, the chloropleth maps show blocks with similar characteristics located near each other in space.
...
The model has some limitations. As stated before in the exploratory analysis, all of the predictor variables apart from the number of households in poverty did not exhibit normality but could not be log-transformed due to their spike at zero. This violates the assumption of normality among variables. This is one severe limitation to the model, because the formula for linear regression cannot be derived when variables are not normal.
