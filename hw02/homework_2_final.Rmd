...
The purpose of this report is to use spatial lag, spatial error and geographically weighted regression to see whether these methods perform better than OLS.


```{r load_data, echo=F, include=F}

filepath <- "C:/Users/sofia/Downloads/"
regdata <- st_read(file.path(filepath, "RegressionDataShp", "RegressionData.shp"))

```


```{r recreate LNNBELPOV100, echo=FALSE, include=FALSE}
...
1. **Homoscedasticity:** This assumption is tied to the independence of errors. In GeoDa/R, tests such as the Breusch-Pagan test and the White test are commonly used to examine data for heteroscedasticity. The null hypothesis (H₀) for these tests states that there is homoscedasticity (constant variance of errors), while the alternative hypothesis (H₁) states that heteroscedasticity is present (non-constant variance of errors).

2. **Normality of Errors:** The assumption of normality of errors can be tested using the Shapiro-Wilk test in GeoDa/R. The null hypothesis (H₀) for this test states that the errors are normally distributed, while the alternative hypothesis (H₁) posits that the errors are not normally distributed.
...
```{r queen_matrix, warning=FALSE, message=FALSE, cache=FALSE}
queen<-poly2nb(regdata, row.names=regdata$POLY_ID)

queenlist<-nb2listw(queen, style = 'W')

moran_value <- moran(regdata$LNMEDHVAL, queenlist, n=length(queenlist$neighbours), S0=Szero(queenlist))$`I` 

result <- paste("The Global Moran's I value for the log of median household value variable (LNMEDHVAL) is about ", round(moran_value,2),".")

print(result)
```


```{r moran_value_LMMEDHVAL, warning=FALSE, message=FALSE, cache=FALSE}
...
```{r moran_hist_LMMEDHVAL, warning=FALSE, message=FALSE, cache=FALSE, fig.cap="Figure 1."}
...
ggplot(moran_data, aes(x = Moran_I)) +
  ...
  labs(title = "Distribution of Moran's I of Spatially Random Median House Value",
       x = "Moran's I Values",
       y = "Frequency") +
  theme_minimal()
```
...
```{r local_morans_I, warning=FALSE, message=FALSE, cache=FALSE}
#Run local moran's I
...
```{r LISA_p_plot, warning=FALSE, message=FALSE, cache=FALSE, fig.cap="Figure 2."}
...
```{r LISA_p_plot_II, fig.width=7,fig.height=7, fig.cap="Figure 3."}
...
Both of the tests for heteroscedasticity (Breusch-Pagan and Koenker-Bassett tests) are non-zero (162.9 and 61.7, respectively) with statistically significant p-values (p < 0.001). This means that we observe heteroscedasticity, where the variance in our OLS residuals is not constant and depends on the predictors.
...
```{r spatially_weighted_resids, message=FALSE, cache=FALSE}
...
```{r scatter_resids, warning=FALSE, message=FALSE, cache=FALSE}
...
moran_plot_ggplot <- function(standardised, listw) {
  ...
  gg <- ggplot(df, aes(x = spatial_lag, y = residuals)) +
    ...
    labs(title = "OLS Residuals by Spatially Weighted Residuals",
         x = "Spatially Weighted Residuals",
         y = "OLS Residuals") +
    theme_minimal() +
...
# Call the function
moran_plot_ggplot(standardised,queenlist)

```

In this scatterplot of the OLS_RESIDU by WT_RESIDU (Figure 4.), we observe a clear positive correlation between the two variables.

```{r regressing_resids, warning=FALSE, message=FALSE, cache=FALSE, fig.cap="Figure 4."}
...
The slope b is 0.268 with p < 0.001, meaning that there is significant spatial autocorrelation that is slightly positive. This makes sense looking at Figure 4.

```{r moran_ols_value, warning=FALSE, message=FALSE, cache=FALSE}
...
```{r ols_morans_hist, warning=FALSE, message=FALSE, cache=FALSE, fig.cap = "Figure 5."}
...
```{r spatial_lag_mod, warning=FALSE, message=FALSE, cache=FALSE}
...
The Breusch-Pagan test results in a value of 220.39 (GeoDa) with p < 0.001, signifying that the spatial lag regression residuals are still heteroscedastic.
...
```{r lag_resids warning=FALSE, message=FALSE, cache=FALSE}
regdata$LAG_RESIDU = lagreg$residuals/sd(lagreg$residuals)
```


```{r res morans value, warning=FALSE, message=FALSE, cache=FALSE}
...
```{r res morans hist, warning=FALSE, message=FALSE, cache=FALSE, fig.cap = "Figure 6."}
...
ggplot(moran_data_lag, aes(x = Moran_I)) +
  ...
  geom_vline(aes(xintercept = moran(regdata$LAG_RESIDU, queenlist, 
                                     ...
                                     S0 = Szero(queenlist))$`I`), 
             color = "red", linetype = "dashed", size = 1) +
  labs(title = "Distribution of Moran's I of Spatially Random Spatial Lag Model Residuals",
       x = "Moran's I Values",
       y = "Frequency") +
  theme_minimal()
```
...
```{r spatial_lag_plot, warning=FALSE, message=FALSE, cache=FALSE, fig.cap="Figure 7."}
...
The Breusch-Pagan test results in a value of 210.99 (GeoDa) with p < 0.001, meaning the spatial error regression residuals are still heteroscedastic.
...
```{r spatial_err_moran_val, warning=FALSE, message=FALSE, cache=FALSE}
...
ggplot(moran_data_err, aes(x = Moran_I)) +
  ...
  geom_vline(aes(xintercept = moran(regdata$ERR_RESIDU, queenlist, 
                                     ...
                                     S0 = Szero(queenlist))$`I`), 
             color = "red", linetype = "dashed", size = 1) +
  labs(title = "Distribution of Moran's I of Spatially Random Spatial Error Model Residuals",
       x = "Moran's I Values",
       y = "Frequency") +
  theme_minimal()
```
...
```{r warning=FALSE, message=FALSE, cache=FALSE, echo=FALSE, include=FALSE}
#Setting an adaptive bandwidth
shps <- as(regdata, 'Spatial')  #These analyses are easier to do when the data are of the SpatialPolygonsDataFrame class
class (shps)
...
```{r gwr_moran_value warning=FALSE, message=FALSE, cache=FALSE}
...
ggplot(moran_data_gwr, aes(x = Moran_I)) +
  ...
  geom_vline(aes(xintercept = moran(regdata$GWR_RESIDU, queenlist, 
                                     ...
                                     S0 = Szero(queenlist))$`I`), 
             color = "red", linetype = "dashed", size = 1) +
  labs(title = "Distribution of Moran's I of Spatially Random GWR Model Residuals",
       x = "Moran's I Values",
       y = "Frequency") +
  theme_minimal()
```
...
```{r gwr_maps, warning=FALSE, message=FALSE, cache=FALSE, fig.cap="Figure 12"}
...
```{r gwr_r2_map, warning=FALSE, message=FALSE, cache=FALSE, fig.cap="Figure 13."}
