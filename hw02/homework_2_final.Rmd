...
#amy wd
queen <- read.gal("Lecture 1 - RegressionData.shp (2)/RegressionData.queen.gal",
                  region.id=NULL, override.id=FALSE)
#queen <- read.gal("data/HW02_RegressionData.gal", 
#                  region.id = NULL,
#                  override.id = FALSE)
queenlist <- nb2listw(queen, style = 'W')

regdata <- st_read("data/Lecture 1 - RegressionData.shp/RegressionData.shp")
#regdata <- st_read("data/HW02_RegressionData.shp") 
...
$$
\text{Moran's I} = \frac{N}{\sum_{i} \sum_{j} w_{ij}} \frac{\sum_{i} \sum_{j} w_{ij} (x_i - \bar{x})(x_j - \bar{x})}{\sum_{i} (x_i - \bar{x})^2}
$$
...
- $Wy$ is the spatially lagged dependent variable, which captures the influence of neighboring values of $y$.
...
moranSig.plot<-function(df,listw, title){
  ...
  tm<-tm_shape(moran.map)+
    ...
    tm_polygons('Pr.z....E.Ii..', style='fixed',breaks=c(0,0.001, 0.01, 0.05, 1), 
            title= 'Local Moran\'s I Significance Map', palette = '-BuPu')+
...
result <- paste0("The Global Moran's I value for OLS residuals is ", round(moranMC_ols$statistic,2),", with a p-value of about ", round(moranMC_ols$p.value,5),".")
...
```{r ols_moran_scatter, message=FALSE, fig.cap = "Figure 7."}
# Function to plot Moran's I
moran_plot_ggplot <- function(standardised, listw) {
  # Calculate Moran's I
  moran_result <- moran.test(standardised, listw)
  ...
  gg <- ggplot(df, aes(x = spatial_lag, y = residuals)) +
    geom_point(alpha = 0.6) +  # Scatter plot
    geom_smooth(method = "lm", color = "red") +  # Linear fit
    labs(title = "OLS Residuals Moran's I Scatter Plot",
         x = "OLS Residuals",
...
result <- paste0("The Global Moran's I value for the residuals for the spatial lag model is about ", round(lagMoranMc$statistic,2),", with a p-value of about ", round(lagMoranMc$p.value,5),".")

print(result)
```


```{r lag_morans_hist, warning=FALSE, message=FALSE, fig.cap = "Figure 8."}
...
```{r spatial_lag_plot, warning=FALSE, message=FALSE, fig.cap="Figure 9."}
...
moran_plot_ggplot <- function(standardised, listw) {
  ...
  gg <- ggplot(df, aes(x = spatial_lag, y = residuals)) +
    geom_point(alpha = 0.6) +  # Scatter plot
    geom_smooth(method = "lm", color = "red") +  # Linear fit
    labs(title = "Moran's I Plot of Spatial Lag REsiduals",
         x = "Lag Residuals",
         y = "Spatially Lagged Lag Residuals") +
    theme_minimal() +
    theme(plot.title = element_text(hjust = 0.5))  # Center the title
...
```{r spatial_err_moran_hist, fig.cap="Figure 10."}
...
moran_plot_ggplot <- function(standardised, listw) {
  ...
  gg <- ggplot(df, aes(x = spatial_lag, y = residuals)) +
    geom_point(alpha = 0.6) +  # Scatter plot
    geom_smooth(method = "lm", color = "red") +  # Linear fit
    labs(title = "Moran's I Plot of Spatial Error Model Residuals",
         x = "Spatial Error Reg. Residuals",
         y = "Spatially Lagged Spatial Error Residuals") +
    theme_minimal() +
    theme(plot.title = element_text(hjust = 0.5))  # Center the title
...
```{r defining_bw, warning=FALSE, message=FALSE, cache=TRUE, echo=TRUE,results="hide"}
...
```{r gwr_model, warning=FALSE, message=FALSE, cache=TRUE}
...
result <- paste0("The Global Moran's I value for the residuals for the spatial gwr model is about ", round(GwrMoranMc$statistic,2),", with a p-value of about ", round(GwrMoranMc$p.value,3),".")

print(result)
```

```{r gwr_moran_hist, fig.cap="Figure 12."}
...
```{r gwr_moran_scatter, fig.cap="Figure 13."}
...
```{r gwr_maps, warning=FALSE, message=FALSE, fig.cap="Figure 14", fig.height=8}
...
PCTVACANT <- tm_shape(shps.sf)+
  tm_polygons(fill="coefPCTVACANTst", col="grey60", lwd = 0.2,
  ...
  tm_title(text="Percent vacant housing units",  fontface="bold", fontfamily="Palatino",
           position = tm_pos_out("center", "top"))

LNNBELPOV <- tm_shape(shps.sf)+
  tm_polygons(fill="coefLNNBELPOV100st",col="grey60", lwd = 0.2,
              fill.legend = tm_legend(title = "", position = tm_pos_in("right", "bottom")),
              tm_scale_intervals(values = "tableau.red_blue_white_diverging",
              breaks=c(-Inf, -2, 0, 2, Inf)))+
  tm_title(text="Number of Households in Poverty (Log)", fontface="bold", fontfamily="Palatino",
           position = tm_pos_out("center", "top"))

PCTBACH <- tm_shape(shps.sf)+
  tm_polygons(fill="coefPCTBACHMORst", col="grey60", lwd = 0.2,
              fill.legend = tm_legend(title = "", position = tm_pos_in("right", "bottom")),
              tm_scale_intervals(values = "tableau.red_blue_white_diverging",
                                 breaks=c(-Inf, -2, 0, 2, Inf)))+
  tm_title(text="Percent of residents with Bachelor's  \nDegree or higher", fontface="bold", fontfamily="Palatino",
           position = tm_pos_out("center", "top"))

PCTSINGLES <- tm_shape(shps.sf)+
  tm_polygons(fill="coefPCTSINGLESst",col="grey60", lwd = 0.2,
              fill.legend = tm_legend(title = "", position = tm_pos_in("right", "bottom")),
              tm_scale_intervals(values = "tableau.red_blue_white_diverging",
                                 breaks=c(-Inf, -2, 0, 2, Inf)))+
  tm_title(text="Percent of detached single-family homes", fontface="bold", fontfamily="Palatino",
           position = tm_pos_out("center", "top"))

tmap_arrange(PCTVACANT, LNNBELPOV, PCTBACH, PCTSINGLES)
```
...
```{r gwr_r2_map, warning=FALSE, message=FALSE, fig.cap="Figure 15."}
...
#tm_shape(shps.sf)+
 # tm_polygons(fill="localR2", title='Local R Squared of Globally Weighted Regression', 
  #        tm_scale_intervals(values = "tableau.red_blue_white_diverging", breaks = c(0,0.27,0.4, 0.53,0.67, Inf)))+
  #tm_layout(frame=FALSE, title = 'GWR Model R Squared Values')

tm_shape(shps.sf)+
  tm_polygons("localR2", col="grey60", lwd = 0.2,
              fill.legend = tm_legend(title = "", position = tm_pos_in("right", "bottom")),
              tm_scale_intervals(values = "carto.peach", breaks = c(0,0.27,0.4, 0.53,0.67, Inf)))+
  tm_title(text="Local R Squared of Globally Weighted Regression", size=1.2, fontface="bold", fontfamily="Palatino",
           position = tm_pos_out("center", "top"))
```
