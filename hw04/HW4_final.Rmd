...
```{r plotKFunction, fig.cap="Fig 3. Ripley's K-Function with Confidence Envelopes"}
...
As shown in figure 3 above, the observed K-function is consistently above the expected K-function and the 99% envelopes. More specifically, until a distance of about 3,000 ft, we observe a random spatial pattern, but for distances greater than 3,000 ft, the observed $\hat{K}$ is greater than the theoretical $K$
...
```{r LFunction_prettierplot, fig.cap="Fig 4. Ripley's L-Function with Confidence Envelopes"}
L2 <- Lenv 
L2$obs <- L2$obs-L2$r
...
```{r FM_map_income, fig.cap="Fig 5. Philadelphia Farmers Markets and Median HH Income by ZIP Code"}
# create map
ggplot() +
